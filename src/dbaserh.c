...
void print_usage() {
  ...
  printf("\t -diff\tPrint differential spectra rather than cumulative\n");
  ...
  printf("\t -gz on/off\t Zero stabilization on/off\n");
  ...
  printf("\t -set zero val\t (Zero adjust, -65534 to 65535 (~-/+32chns)\n");
  printf("\t -set lld  val\t (Lower level discriminator, 0-1023)\n");
  printf("\t -set uld  val\t (upper level discriminator, 0-1023)\n");
  printf("\t -set pw   val\t (Pulse width, 0.75-2.0 us)\n");
  printf("\t -set ltp  val\t (Live time preset (s))\n");
  printf("\t -set rtp  val\t (Real time preset (s))\n");
  printf("\t -set gsch C W\t (Gain stab center ch. (1-1022) and width (2-1024))\n");
  printf("\t -set zsch C W\t (Zero stab center ch. (1-1022) and width (2-1024))\n");
...
int main(int argc, char **argv) {
  ...
  for(k=1; k < argc; k++){
    ...
    /* Gain stab. on/off */
    else if(strcmp(argv[k],"-gs") == 0)
      {
        ...
        gs = argv[k+1];
        if(strcmp(gs,"on") != 0 && strcmp(gs,"off") != 0){ /* fix RTS 6/2/2023 */
          if(!q)
  ...
  */
  ...
  if(1 == opts.haveopts){
    ...
    /* zero adjust set? */
    if(opts.zero != -65535){
      if(libdbase_set_zero(det, opts.zero) < 0 && !q)
        fprintf(stderr, "E: setting zero adjust\n");
      else if(!q)
        printf("Setting Zero to %5d\n", opts.zero);
    }
    /* lower level discriminator set? */
    if(opts.lld != -1){
      if(libdbase_set_lld(det, opts.lld) < 0 && !q)
        fprintf(stderr, "E: setting lower level discriminator\n");
      else if(!q)
        printf("Setting LLD to %4d\n", opts.lld);
    }
    /* upper level discriminator set? */
    if(opts.uld != -1){
      if(libdbase_set_uld(det, opts.uld) < 0 && !q)
        fprintf(stderr, "E: setting upper level discriminator\n");
      else if(!q)
        printf("Setting ULD to %4d\n", opts.uld);
    }
    /* gain stabilizer channels set? */
    if(opts.gsch[0] > 0){
      if(libdbase_set_gs_chans(det, opts.gsch[0], opts.gsch[1]) < 0 && !q)
        fprintf(stderr, "E: setting gain stabilizer channels\n");
      else if(!q)
        printf("Setting gain stabilizer center %d width %d\n", opts.gsch[0] , opts.gsch[1]);
    }
    /* zero stabilizer channels set? */
    if(opts.zsch[0] > 0){
      if(libdbase_set_zs_chans(det, opts.zsch[0], opts.zsch[1]) < 0 && !q)
        fprintf(stderr, "E: setting zero stabilizer channels\n");
      else if(!q)
        printf("Setting zero stabilizer center %d width %d\n", opts.zsch[0] , opts.zsch[1]);
    }
...
int parse_settings(const char* set, char** val,
  ...
  else if(0 == strcmp(set, "zero")){
    opts->zero = atoi(val[pos]);
    err = 0;
  }
  else if(0 == strcmp(set, "lld")){
    opts->lld = atoi(val[pos]);
    err = 0;
  }
  else if(0 == strcmp(set, "uld")){
    opts->uld = atoi(val[pos]);
    err = 0;
  }
